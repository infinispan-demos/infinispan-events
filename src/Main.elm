...
import Events exposing
  (ConferenceTalk, appendTalk, talksDecoder, viewTalk, viewTalkLabel)
...
model =
  ...
  , talkLink = "http://www.devnation.org/#50856"
  -- , talkTitle = ""
  -- , talkSpeaker = ""
  -- , talkConf = ""
  -- , talkLoc = ""
...
performInsertTalk talk =
  let
    url =
      "http://localhost:3000/events"
  in
    Task.perform InsertTalkFail InsertTalkSucceed (Http.post decodeInsertedTalk url talk)
...
viewEventsUpcoming model =
  case model.talks of
    Just talks ->
      let
          talkViews = List.map viewTalk talks
      in
    ...
    Nothing ->
      let
        (color, message) = model.talksError
      in
...
viewQueryResult model =
  case model.queryResults of
    ...
    Nothing ->
      let
        (color, message) = model.queryError
      in
        if not (String.isEmpty message) then
